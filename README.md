...
Tron 区块链事件监听服务，实时解析 Tron 链上区块，筛选出你关注的地址的转账事件，并通过 NATS JetStream 推送给下游消费者。
...
详见 `config.yaml` 中的注释，所有选项均可通过 `TRONECHO_*` 环境变量覆盖。
...
编辑 `config.yaml`，填入你的 RPC 节点地址和 NATS 地址。
